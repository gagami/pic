...
# 交互部分
# -----------------------------
echo -e "\033[1;34m=== VPS 初始化优化脚本 ===\033[0m"

read -p "请输入新主机名 (默认: vps-host): " NEW_HOSTNAME
NEW_HOSTNAME=${NEW_HOSTNAME:-vps-host}
...
read -p "请选择要安装的 XanMod 内核类型 (main/edge): " KERNEL_TYPE
KERNEL_TYPE=${KERNEL_TYPE,,}
[[ "$KERNEL_TYPE" != "main" && "$KERNEL_TYPE" != "edge" ]] && { echo "输入无效，仅支持 main 或 edge"; exit 1; }
...
fallocate -l "${SWAP_SIZE}M" "$SWAP_FILE" || dd if=/dev/zero of="$SWAP_FILE" bs=1M count="$SWAP_SIZE" status=none
chmod 600 "$SWAP_FILE"
...
# ✅ 正确检测 ABI
ABI=$(wget -qO- https://dl.xanmod.org/check_x86-64_psabi.sh | bash | head -n1 | tr -d '\r')
if [[ -z "$ABI" ]]; then
    echo -e "\033[33m[警告]\033[0m 未检测到 ABI，默认使用 x86-64-v2。"
    ABI="x86-64-v2"
fi
