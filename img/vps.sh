...
# ==============================
# 自动检测 ABI 并安装 XanMod 内核（自研检测，无需下载脚本）
# ==============================
...
# v2: SSE4.2 + POPCNT
# v3: AVX + FMA
# v4: AVX2 + BMI1/BMI2 + FMA + MOVBE

ABI=""

if [[ "$CPU_FLAGS" =~ avx2 ]] && [[ "$CPU_FLAGS" =~ bmi1 ]] && [[ "$CPU_FLAGS" =~ bmi2 ]] && [[ "$CPU_FLAGS" =~ fma ]] && [[ "$CPU_FLAGS" =~ movbe ]]; then
    ABI="x86-64-v4"
elif [[ "$CPU_FLAGS" =~ avx ]] && [[ "$CPU_FLAGS" =~ fma ]]; then
    ABI="x86-64-v3"
elif [[ "$CPU_FLAGS" =~ sse4_2 ]] && [[ "$CPU_FLAGS" =~ popcnt ]]; then
    ABI="x86-64-v2"
else
    ABI="x86-64-v1"
fi
...
if ! apt-cache show "$PKG" >/dev/null 2>&1; then
    echo -e "\033[31m[ERROR]\033[0m 找不到对应的 XanMod 包 ($PKG)。请检查网络或更换为 main 版本再试。"
    exit 1
fi
