...
# ==============================
echo "正在检测系统 ABI 以选择正确的 XanMod 内核..."

TMP_ABI_SCRIPT="/tmp/check_x86-64_psabi.sh"

# 下载检测脚本
wget -qO "$TMP_ABI_SCRIPT" https://dl.xanmod.org/check_x86-64_psabi.sh
if [[ ! -s "$TMP_ABI_SCRIPT" ]]; then
    echo "[ERROR] 下载 ABI 检测脚本失败！"
    exit 1
fi
chmod +x "$TMP_ABI_SCRIPT"

# 尝试官方 shell 执行检测
ABI=$(/bin/bash "$TMP_ABI_SCRIPT" 2>/dev/null | head -n1 | tr -d '\r')

# 如果 shell 执行失败或为空，则 fallback awk 解析
if [[ -z "$ABI" ]]; then
    echo "[WARN] 官方 ABI 检测失败，尝试用 awk 解析脚本内容提取 ABI..."
    ABI=$(awk '/x86-64-v[1-4]/ {print $1; exit}' "$TMP_ABI_SCRIPT")
fi

# 如果仍为空，则直接报错退出
if [[ -z "$ABI" ]]; then
    echo -e "\033[31m[ERROR]\033[0m 无法检测到系统 ABI，无法继续安装 XanMod 内核！"
    exit 1
fi
...
case "$ABI" in
    *x86-64-v1*)
        [[ "$XANMOD_KERNEL_TYPE" == "edge" ]] && { 
            echo -e "\033[33m[WARN]\033[0m v1 架构不支持 Edge 内核，自动切换为 main."
            XANMOD_KERNEL_TYPE="main"
        }
        PKG="linux-xanmod-x64v1"
        ;;
    *x86-64-v2*) PKG="linux-xanmod-${XANMOD_KERNEL_TYPE}-x64v2" ;;
    *x86-64-v3*) PKG="linux-xanmod-${XANMOD_KERNEL_TYPE}-x64v3" ;;
    *x86-64-v4*) PKG="linux-xanmod-${XANMOD_KERNEL_TYPE}-x64v4" ;;
    *) 
        echo -e "\033[31m[ERROR]\033[0m 未能识别 ABI ($ABI)，无法继续安装 XanMod 内核！"
        exit 1
        ;;
esac
...
if ! apt-cache show "$PKG" >/dev/null 2>&1; then
    echo -e "\033[31m[ERROR]\033[0m 找不到对应的 XanMod 包 ($PKG)。"
