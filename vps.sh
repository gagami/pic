...
if [[ "$OS" == "debian" ]]; then
    ...
    for interface in $interfaces; do
        ...
        nmcli connection modify $interface txqueuelen 10000

        # 调优网络设备积压队列以避免数据包丢弃
        echo "Tuning network device backlog for interface $interface..."
        nmcli connection modify $interface rxqueuelen 10000

        # 增加NIC的传输队列长度
        echo "Increasing NIC transmission queue length for interface $interface..."
        nmcli connection modify $interface transmit-hash-policy layer2+3
    done
fi
