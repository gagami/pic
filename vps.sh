...
# ==============================
# 自研 ABI 检测 + 安装 XanMod 内核
# ==============================
log "正在检测系统 ABI 并安装 XanMod 内核..."
...
read -p "请选择 XanMod 内核类型 [main/edge]: " XANMOD_KERNEL_TYPE
if [[ ! "$XANMOD_KERNEL_TYPE" =~ ^(main|edge)$ ]]; then
    echo "[ERROR] 内核类型输入错误，只能为 main 或 edge"
    exit 1
fi

if [[ "$ABI" == "x86-64-v1" ]] && [[ "$XANMOD_KERNEL_TYPE" == "edge" ]]; then
    echo "[WARN] v1 架构不支持 edge 内核，自动切换为 main"
