#!/bin/bash
# ==========================================================
# Watchtower Monitor 安装脚本 v12.6（最终版）
# ==========================================================
# 功能：
...
echo "=============================="
echo " Watchtower Monitor 安装脚本 v12.6 "
echo "=============================="
...
# -----------------------------
# 清理脚本（修正版，Telegram 时间正确显示） 
# -----------------------------
cat > "$CLEAN_SCRIPT" <<'EOF'
#!/bin/bash
# Docker 日志清理报告（修正版，时间和日志正确显示）
...
docker ps -q | while read -r cid; do
    ...
    LOCAL_ID=$(docker images --format '{{.Repository}}:{{.Tag}} {{.ID}}' | grep "^$IMAGE" | awk '{print $2}')
    docker pull -q "$IMAGE" >/tmp/docker_pull_check.log 2>&1
    REMOTE_ID=$(docker images --format '{{.Repository}}:{{.Tag}} {{.ID}}' | grep "^$IMAGE" | awk '{print $2}')
    if [ "$LOCAL_ID" != "$REMOTE_ID" ]; then
        UPDATES+="$IMAGE\n"
    fi
done

# 构建 Telegram 消息（代码块显示）
