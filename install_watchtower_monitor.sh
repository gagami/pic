...
# ==========================================================
# 功能：
# ✅ 每小时检测镜像更新（Monitor-only 模式）
# ✅ Telegram 通知（含主机名）
# ✅ 容器异常退出/重启通知
# ✅ 日志写入 /root/watchtower-monitor/updates.log
# ✅ 每天凌晨 4:00 自动清理旧镜像、容器、网络
# ✅ 自动保留最近 30 天日志
# ✅ 清理后发送 Telegram 报告（包含释放空间信息）
# ==========================================================
...
services:
  watchtower:
    ...
    environment:
      ...
      - WATCHTOWER_NOTIFICATION_TEMPLATE=*{{range .Entries}}🚀 容器事件通知 🚀%0A\
主机: ${HOSTNAME}%0A\
容器: {{.ContainerName}}%0A\
镜像: {{.ImageName}}%0A\
状态: {{.State}}%0A\
旧版: {{.CurrentImageID}}%0A\
新版: {{.NewImageID}}%0A\
镜像构建时间: {{.ImageCreatedAt}}%0A\
检测时间: {{.UpdatedAt}}%0A\
模式: Monitor-only (仅检测)%0A\
------------------------------------%0A\
📜 日志已写入 /root/watchtower-monitor/updates.log%0A{{end}}*
    command: --no-color
EOF
...
# -----------------------------
echo
echo "✅ Watchtower Monitor 部署完成（含清理报告）！"
echo "----------------------------------------"
